...
import zhy2002.leetcode.solutions.besttimetobuyandsellstockiv.*;
...
public class BestTimeToBuyAndSellStockIVTests {
    ...
    @Test
    public void properDpTest(){
        Solution solution = new ProperDpSolution();
        basicTest(solution);
        failedOjTest(solution);
    }

    @Test
    public void tleDpTest(){
        Solution solution = new TleDpSolution();
        basicTest(solution);
        failedOjTest(solution);
    }
    ...
    public void randomComparison(){
        Solution solution2 = new ProperDpSolution();
        Solution solution1 = new TleDpSolution();
        int[] input = Utils.randomIntArray(15, 10, 21);
        int result1 =  solution1.maxProfit(3, input);
        int result2 =  solution2.maxProfit(3, input);
        assertThat(result1, equalTo(result2));

        Utils.randomIntArray(10, 5, 14);
        result1 =  solution1.maxProfit(2, input);
        result2 =  solution2.maxProfit(2, input);
        assertThat(result1, equalTo(result2));
    }

    private void basicTest(Solution solution){

        assertThat(solution.maxProfit(2, new int[]{6, 1, 3, 2, 4, 7}), equalTo(7));
        assertThat(solution.maxProfit(4, new int[]{1, 2, 4, 2, 5, 7, 2, 4, 9, 0}), equalTo(15));
        assertThat(solution.maxProfit(6, new int[]{8, 3, 6, 2, 8, 8, 8, 4, 2, 0, 7, 2, 9, 4, 9}), equalTo(28));
        assertThat(solution.maxProfit(5, new int[]{5, 2, 3, 2, 6, 6, 2, 9, 1, 0, 7, 4, 5, 0}), equalTo(20));
        assertThat(solution.maxProfit(2, new int[]{9, 3, 4, 9, 5, 1}), equalTo(6));
        assertThat(solution.maxProfit(2, new int[]{}), equalTo(0));
        assertThat(solution.maxProfit(2, new int[]{1, 2}), equalTo(1));
        assertThat(solution.maxProfit(2, new int[]{2, 1}), equalTo(0));
        assertThat(solution.maxProfit(2, new int[]{2, 2, 5}), equalTo(3));
        assertThat(solution.maxProfit(11, new int[]{2, 2, 5, 3, 6, 1, 4}), equalTo(9));
    }

    private void failedOjTest(Solution solution){
        int k = 6;
        InputStream stream = this.getClass().getResourceAsStream(this.getClass().getSimpleName() + "_failedOjTest.txt");
